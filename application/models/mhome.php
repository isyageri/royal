...
class mHome extends CI_Model {
	...
	public function GetMenu($menu=0) {
		$pid = empty($menu) ? "is null" : "= $menu";
		$uid = empty($uid)? 0 : $uid;
		$sql = "SELECT * FROM t_menu WHERE MenuPID $pid ORDER BY MenuID, MenuPID";
    ...
    public function GetMenuWhatWeOffer(){
        $sql = "SELECT * FROM t_menu WHERE MenuPID = 8 ORDER BY MenuID, MenuPID";
        $dat = rst2Array($sql);
        return $dat;
    }
	...
	public function get_privilege($gid, $menu) {
		$gid = empty($gid)? '' : $gid;
		$sql = "select p_menu, p_create, p_retrieve, p_update, p_delete
				from t_privilege
				where usergroup_id = $gid
				and menu_uid = $menu";
