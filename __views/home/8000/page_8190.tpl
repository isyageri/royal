	...
	<form id="fsearch" class="long">
		<fieldset class="expose post_extend" >
			<legend>Data Filter</legend>
				...
				<label >Tanggal Produksi Produk</label><input type="text" class="postit date_distribution_awal date"  value="" />
				s/d
				<input type="text" class="postit date_distribution_akhir date"  value="" />
				<br />
...
<div class="title-rpt">
		<legend>Laporan Produksi Produk</legend>
		<div>	
			<table id="t{$menu}desc" class="plus_fetch t{$menu}desc 8197">
					<tr><td>Period</td><td> : </td><td >{$periodlabel}</td></tr>
					<tr><td>Tanggal Generate</td><td> : </td><td>{$cur_date}</td></tr>
			</table>
			<br/>
			{if !$tblo}
			<a href="#t{$menu}" class="easyui-linkbutton" onclick="dwld_rpt(8190)" plain="true" iconCls="icon-xls">Export</a>
			{/if}
		</div>	
	</div>
