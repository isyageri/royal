	...
	<form id="fsearch" class="long">
		<fieldset class="expose post_extend" >
			<legend>Data Filter</legend>
				...
				<label >Tanggal Pengeluaran Produksi</label><input type="text" class="postit date_pengeluaran_produksi_awal date"  value="" />
				s/d
				<input type="text" class="postit date_pengeluaran_produksi_akhir date"  value="" />
				<br />
...
{/if}
<div class="title-rpt">
		<legend>Laporan Pengeluaran Produksi</legend>
		<div>	
			<table id="t{$menu}desc" class="plus_fetch t{$menu}desc 8187">
					<tr><td>Period</td><td> : </td><td >{$periodlabel}</td></tr>
					<tr><td>Tanggal Generate</td><td> : </td><td>{$cur_date}</td></tr>
			</table>
			<br/>
			{if !$tblo}
			<a href="#t{$menu}" class="easyui-linkbutton" onclick="dwld_rpt(8180)" plain="true" iconCls="icon-xls">Export</a>
			{/if}
		</div>	
</div>
