	...
	<form id="fsearch" class="long">
		<fieldset class="expose post_extend" >
			<legend>Data Filter</legend>
				...
				<label >Tanggal Pembelian Barang</label><input type="text" class="postit date_alur_barang_awal date"  value="" />
				s/d
				<input type="text" class="postit date_alur_barang_akhir date"  value="" />
				<br />
...
<div class="title-rpt">
		<legend>Laporan Pembelian Barang</legend>
		<div>	
			<table id="t{$menu}desc" class="plus_fetch t{$menu}desc 8167">
					<tr><td>Period</td><td> : </td><td >{$periodlabel}</td></tr>
					<tr><td>Tanggal Generate</td><td> : </td><td>{$cur_date}</td></tr>
			</table>
			<br/>
			{if !$tblo}
			<a href="#t{$menu}" class="easyui-linkbutton" onclick="dwld_rpt(8160)" plain="true" iconCls="icon-xls">Export</a>
			{/if}
		</div>	
	</div>
